...
Third-party player ratings are auxiliary data, not a hard collection gate. Prefer named same-source providers for a whole match or team packet. If a provider is unavailable, paywalled, dynamically inaccessible, stale, or does not cover all relevant players, preserve the gap with `rating_source_status` and `rating_missing_note`.

Do not blend ratings from multiple providers as if they were the same scale. Do not create a subjective `external_rating`. If a derived internal proxy is useful, label it as `internal_rating_proxy` and state the observable basis.
